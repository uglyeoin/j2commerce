...
$function  = $app->input->getCmd('function', 'jSelectCurrency');
...
<div class="container-popup">
    <form action="<?php echo Route::_('index.php?option=com_j2commerce&view=currencies&layout=modal&tmpl=component&function=' . $function . '&' . Session::getFormToken() . '=1'); ?>" method="post" name="adminForm" id="adminForm">
            ...
            <table class="table table-sm">
                ...
                <?php foreach ($this->items as $i => $item) : ?>
                    <tr class="row<?php echo $i % 2; ?>">
                        <th scope="row">
                            <a class="select-link" href="javascript:void(0)" data-function="<?php echo $onclick; ?>" data-id="<?php echo (int) $item->j2commerce_currency_id; ?>" data-title="<?php echo $this->escape($item->currency_title); ?>">
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.select-link').forEach(function(element) {
        element.addEventListener('click', function(event) {
            ...
            const currencyTitle = this.getAttribute('data-title');

            if (window.parent[functionName]) {
                window.parent[functionName](currencyId, currencyTitle);
            }

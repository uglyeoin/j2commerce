...
class ReportsModel extends ListModel
{
    ...
    protected function getListQuery()
    {
        ...
        // Filter to show only report methods (plugins that start with report_)
        $query->where($db->quoteName('a.element') . ' LIKE ' . $db->quote('report_%'));
    ...
    {
        return $this->loadForm('com_j2commerce.reports.filter', 'filter_reports', ['control' => '', 'load_data' => $loadData]);
    }

...
class ReportModel extends AdminModel
{
     ...
     * @since  6.0.0
     */
    public $typeAlias = 'com_j2commerce.report';
    ...
    protected function loadFormData()
    {
        ...
        $data = Factory::getApplication()->getUserState('com_j2commerce.edit.report.data', []);

        if (empty($data)) {
            $data = $this->getItem();
        }

        $this->preprocessData('com_j2commerce.report', $data);
    ...
    public function publish(&$pks, $value = 1)
    {
        ...
        foreach ($pks as $i => $pk) {
            if ($table->load($pk)) {
                if (!$user->authorise('core.edit.state', 'com_j2commerce.report.' . $pk)) {
    ...
    public function delete(&$pks)
    {
        ...
        $app->enqueueMessage(Text::_('COM_J2COMMERCE_REPORT_DELETE_NOT_ALLOWED'), 'error');
        return false;
    }
}

...
/**
 * Cart Default Behavior Class
 *
 * Provides base/fallback cart behavior implementation that handles common
 * cart operations and triggers plugin events for products that don't have
 * a specific cart behavior class.
 *
 * @since 6.0.0
 */
class CartDefault
{
     ...
     *
     * Default implementation for adding items to cart. Validates product options,
     * quantity restrictions, and stock availability before adding to cart.
     * Triggers plugin events for custom validation and modifications.
     *
     ...
     *
     * Default implementation for processing cart items. Loads product data,
    ...
    public function onGetCartItems(CartModel &$model, object &$item): void
    {
        ...
        // Build cartitem_params with thumb_image and other data for template display
        $existingParams = [];
        if (!empty($item->cartitem_params)) {
            $existingParams = (array) json_decode($item->cartitem_params, true);
        }

        // Add product image to params (clean Joomla media handler syntax if present)
        $imageUrl = $product->thumb_image ?? $product->main_image ?? '';
        if (!empty($imageUrl)) {
            // Remove Joomla media handler syntax (everything after #)
        ...
        // Get variant: prefer ->variant (master object), fallback to first in ->variants array or object
        $variant = $product->variant ?? null;
        if ($variant === null && !empty($product->variants)) {
        ...
        // Add back_order_item status if applicable
        ...
        if ($variant !== null && \is_object($variant)) {
            ...
            // Fallback: create pricing object from cart item's variant_price
            $item->pricing = (object) [
        ...
        // Trigger plugin event after getting product (from original default.php)
        $pluginHelper = J2CommerceHelper::plugin();
        $pluginHelper->importCatalogPlugins();
        $pluginHelper->event('AfterGetProduct', [&$item]);
    }

    /**
     * Validate cart event handler
     *
     * Default implementation for validating cart items when updating quantities.
     * Checks quantity restrictions and stock availability.
     *
     * @param   CartModel   $model     The cart model instance
     ...
     *
     * Triggers plugin events after retrieving a product/cart item.
     * This provides a hook point for plugins to modify product data.
     *
     * @param   object  $model   The model instance
     * @param   object  $record  The record/item (by reference)
     *
     * @return  void
     *
     * @since   6.0.0
    ...
    {
        // Import and trigger catalog plugins
        $pluginHelper = J2CommerceHelper::plugin();
        $pluginHelper->importCatalogPlugins();

        $app = Factory::getApplication();
        $app->triggerEvent('onJ2CommerceAfterGetProduct', [&$record]);
    }

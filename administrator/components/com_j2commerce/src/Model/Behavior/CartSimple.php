...
/**
 * Cart Simple Product Behavior Class
 *
 * Handles cart lifecycle events for simple products including add to cart,
 * cart item retrieval, and cart validation.
 *
 * @since 6.0.0
 */
class CartSimple
{
     ...
     * before adding a simple product to the cart. Creates the cart item object
     * and triggers plugin events.
     *
    ...
    public function onBeforeAddCartItem(CartModel &$model, object $product, \stdClass &$json): void
    {
        ...
        if (empty($errors) && isset($cart->cart_type) && $cart->cart_type !== 'wishlist') {
            // Get variant: prefer ->variant (master object), fallback to first in ->variants array
        ...
        if (empty($errors)) {
            ...
            // Get variant_id: prefer ->variant (master), fallback to first element of ->variants array
            $variantId = 0;
            ...
            $item->product_type    = $product->product_type ?? 'simple';
     ...
     *
     * Processes a cart item for simple products, loading product data,
    ...
    public function onGetCartItems(CartModel &$model, object &$item): void
    {
        ...
        if ($variant !== null && \is_object($variant)) {
            ...
            // Set taxprofile_id from product (column is on products table, not variants)
     ...
     * updating cart quantities for simple products.
     *
     * @param   CartModel   $model     The cart model instance
    ...
    {
        // Only process simple products
        if (($cartitem->product_type ?? '') !== 'simple') {
            return true;
        }

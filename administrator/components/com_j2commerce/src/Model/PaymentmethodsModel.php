...
class PaymentmethodsModel extends ListModel
{
    ...
    protected function getListQuery()
    {
        ...
        // Filter to show only payment methods (plugins that start with payment_)
        $query->where($db->quoteName('a.element') . ' LIKE ' . $db->quote('payment_%'));
    ...
    public function getItems()
    {
        ...
        if ($items === false || !\is_array($items)) {
            ...
            $app->enqueueMessage(Text::_('COM_J2COMMERCE_PAYMENT_METHODS_NO_PLUGINS_FOUND'), 'warning');
            return [];
        }
        ...
        foreach ($items as $item) {
            ...
            if (!empty($item->manifest_cache)) {
                ...
                if (\is_object($manifest)) {
                    ...
                    // Self-heal corrupted name from manifest
                    if (!empty($manifest->name) && $manifest->name !== $item->name) {
            ...
            // Create edit link
            $item->edit_link = 'index.php?option=com_j2commerce&task=paymentmethod.edit&extension_id=' . $item->extension_id;
        }
    ...
    {
        return $this->loadForm('com_j2commerce.paymentmethods.filter', 'filter_paymentmethods', ['control' => '', 'load_data' => $loadData]);
    }

...
class VendorsModel extends ListModel
{
    ...
    public function __construct($config = [])
    {
        if (empty($config['filter_fields'])) {
            $config['filter_fields'] = [
                'j2commerce_vendor_id', 'a.j2commerce_vendor_id',
                'j2commerce_user_id', 'a.j2commerce_user_id',
    ...
    protected function getListQuery(): QueryInterface
    {
        ...
        // Select required fields from vendors table
        $query->select(
            $db->quoteName([
                'a.j2commerce_vendor_id',
                'a.j2commerce_user_id',
                'a.address_id',
                'a.enabled',
                'a.ordering',
            ])
        ...
        $query->from($db->quoteName('#__j2commerce_vendors', 'a'));

        // Join with users table to get username
        $query->select($db->quoteName('u.username', 'user_name'))
            ->join('LEFT', $db->quoteName('#__users', 'u') . ' ON ' . $db->quoteName('u.id') . ' = ' . $db->quoteName('a.j2commerce_user_id'));

        // Join with addresses table to get company, city, country_id, zone_id
        $query->select(
            $db->quoteName([
                'addr.company',
                'addr.city',
        ...
        if (!empty($search)) {
            if (stripos($search, 'id:') === 0) {
                ...
                $query->where($db->quoteName('a.j2commerce_vendor_id') . ' = :searchId')
                ...
                $query->where(
                    ...
                    $db->quoteName('addr.company') . ' LIKE :search2 OR ' .
                    $db->quoteName('addr.city') . ' LIKE :search3' .
                    ')'
                )
                    ->bind(':search1', $search)
                    ->bind(':search2', $search)

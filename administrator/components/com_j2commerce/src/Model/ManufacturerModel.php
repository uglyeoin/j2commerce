...
class ManufacturerModel extends AdminModel
{
     ...
     * @since  6.0.6
     */
    public $typeAlias = 'com_j2commerce.manufacturer';
     ...
     * looks for a URL parameter named after the table's primary key (j2commerce_manufacturer_id),
     * but Joomla's standard convention uses 'id' as the URL parameter.
     *
     * Without this override:
     * - URL has: ?id=1
     * - Parent looks for: ?j2commerce_manufacturer_id=1 (not found!)
     * - State gets: manufacturer.id = 0
    ...
    protected function populateState(): void
    {
        ...
        // NOT from 'j2commerce_manufacturer_id' (the table's column name)
    ...
    {
        $form = $this->loadForm('com_j2commerce.manufacturer', 'manufacturer', ['control' => 'jform', 'load_data' => $loadData]);
     ...
     * @since   6.0.6
     */
    public function getTable($name = 'Manufacturer', $prefix = 'Administrator', $options = []): Table
    {
    ...
    {
        $app  = Factory::getApplication();
        $data = $app->getUserState('com_j2commerce.edit.manufacturer.data', []);
    ...
    public function save($data): bool
    {
        ...
        if (!empty($data['address_id']) && $data['address_id'] > 0) {
            ...
            // Create new address
            $addressData['user_id'] = 0; // Manufacturers don't have associated users
            $addressData['type']    = 'manufacturer';
            $addressId              = $this->saveAddress($addressData);
            $data['address_id']     = $addressId;
        }

        // Remove address fields from manufacturer data before saving
        foreach ($addressFields as $field) {
            unset($data[$field]);
        }
        foreach ($customFields as $field) {
    ...
    {
        // Ensure address_id and brand_desc_id are integers
        if (!empty($table->address_id)) {
            $table->address_id = (int) $table->address_id;
        }

        $table->brand_desc_id = (int) ($table->brand_desc_id ?? 0);
    }
}

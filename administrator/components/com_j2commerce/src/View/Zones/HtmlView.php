...
namespace J2Commerce\Component\J2commerce\Administrator\View\Zones;
...
class HtmlView extends BaseHtmlView
{
    ...
    protected function addToolbar(): void
    {
        ...
        ToolbarHelper::title(Text::_('COM_J2COMMERCE_ZONES'), 'fa-solid fa-location-dot');

        if ($canDo->get('core.create')) {
            $toolbar->addNew('zone.add');
        }
        ...
        if ($canDo->get('core.delete') && ($this->state->get('filter.enabled') == -2)) {
            $toolbar->delete('zones.delete', 'JTOOLBAR_DELETE_FROM_TRASH')->listCheck(true);
        }

        // Link button to related view
        $toolbar->linkButton('countries')
            ->text('COM_J2COMMERCE_VIEW_COUNTRIES')
            ->url('index.php?option=com_j2commerce&view=countries')
            ->icon('icon-globe');

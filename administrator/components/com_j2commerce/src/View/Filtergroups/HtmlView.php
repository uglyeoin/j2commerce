...
namespace J2Commerce\Component\J2commerce\Administrator\View\Filtergroups;
...
class HtmlView extends BaseHtmlView
{
    ...
    {
        $canDo = ContentHelper::getActions('com_j2commerce', 'filtergroup');
        $user  = Factory::getApplication()->getIdentity();

        // Get the toolbar object instance
        $toolbar = Toolbar::getInstance('toolbar');

        ToolbarHelper::title(Text::_('COM_J2COMMERCE_FILTERGROUPS'), 'filter');

        if ($canDo->get('core.create')) {
            $toolbar->addNew('filtergroup.add');
        }

        // Add "View Products" button for navigation
        $toolbar->linkButton('products')
            ->text('COM_J2COMMERCE_VIEW_PRODUCTS')
        ...
        if (!$this->isEmptyState) {
            if ($canDo->get('core.edit.state')) {
                ...
                $childBar->publish('filtergroups.publish')->listCheck(true);
                $childBar->unpublish('filtergroups.unpublish')->listCheck(true);

                if ($this->state->get('filter.enabled') != -2) {
                    $childBar->trash('filtergroups.trash')->listCheck(true);
                }
            }

...
namespace J2Commerce\Component\J2commerce\Administrator\View\Length;
        ...
        if ($canDo->get('core.edit.state')) {
            ...
            $saveGroup->configure(
                function (Toolbar $childBar) use ($canDo, $isNew, $checkedOut) {
                    if (!$checkedOut && ($canDo->get('core.edit') || $canDo->get('core.create'))) {
                        $childBar->save('length.save');
                    }

                    if ($canDo->get('core.create')) {
                        $childBar->save2new('length.save2new');
                    }

                    if (!$isNew && $canDo->get('core.create')) {
                        $childBar->save2copy('length.save2copy');
                    }
                }
            );
        }

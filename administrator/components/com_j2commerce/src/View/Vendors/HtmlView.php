...
namespace J2Commerce\Component\J2commerce\Administrator\View\Vendors;
...
class HtmlView extends BaseHtmlView
{
    ...
    protected function addToolbar(): void
    {
        ...
        ToolbarHelper::title(Text::_('COM_J2COMMERCE_VENDORS'), 'fas fa-solid fa-user-tag');
        ...
        if (!$this->isEmptyState) {
            if ($canDo->get('core.edit.state')) {
                ...
                if ($this->state->get('filter.enabled') != -2) {
                    $childBar->trash('vendors.trash')->listCheck(true);
                }
            }
        }

        if ($canDo->get('core.delete') && ($this->state->get('filter.enabled') == -2)) {
            $toolbar->delete('vendors.delete', 'JTOOLBAR_DELETE_FROM_TRASH')->listCheck(true);
        }

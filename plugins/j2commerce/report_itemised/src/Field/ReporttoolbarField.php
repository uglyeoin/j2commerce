...
namespace J2Commerce\Plugin\J2Commerce\ReportItemised\Field;
...
class ReporttoolbarField extends FormField
{
    ...
    protected function getInput(): string
    {
        ...
        $app->getLanguage()->load(
            'plg_j2commerce_report_itemised',
            JPATH_PLUGINS . '/j2commerce/report_itemised',
            null,
            true
        );
        ...
        $viewBtn->text('PLG_J2COMMERCE_REPORT_ITEMISED_VIEW_REPORT')
            ->url('index.php?option=com_j2commerce&view=reportplugin&plugin=report_itemised&pluginview=report')
            ->icon('fa-solid fa-chart-bar');
        $toolbar->prependButton($viewBtn);

        $backBtn = new LinkButton('report-back');
        $backBtn->text('PLG_J2COMMERCE_REPORT_ITEMISED_BACK_TO_REPORTS')

...
namespace J2Commerce\Plugin\J2Commerce\ReportItemised\Extension;

use J2Commerce\Component\J2commerce\Administrator\Helper\J2CommerceHelper;
use J2Commerce\Plugin\J2Commerce\ReportItemised\Model\ReportitemisedModel;
...
/**
 * Itemised Report Plugin for J2Commerce
 *
 * Provides an itemised order report grouped by product and options,
 * showing category, quantity, and purchase count with bar chart, filtering,
 * and CSV export. Fully self-contained — model, filter XML, and templates
 * all live in the plugin directory.
 *
 * @since  6.0.0
 */
final class ReportItemised extends CMSPlugin implements SubscriberInterface
{
     ...
     * @since  6.0.0
     */
    private string $_element = 'report_itemised';
    ...
    public function onReportPluginView(Event $event): void
    {
        ...
        $event->setArgument('title', Text::_('PLG_J2COMMERCE_REPORT_ITEMISED'));
     ...
     *
     * Uses the ReportitemisedModel with pagination removed for full export.
     * Includes all columns: Product ID, Product Name, Options, Category,
    ...
    private function buildExportData(): array
    {
        ...
        foreach ($items as $item) {
            ...
            if (!empty($item->attributes)) {
                ...
                foreach ($item->attributes as $attr) {
                    $parts[] = $attr->orderitemattribute_name . ': ' . $attr->orderitemattribute_value;
                }
            ...
            $csvRow->$purchasesCol = $item->order_count;
            $export[]              = $csvRow;
        }
        ...
        $finalRow->$purchasesCol = $orderTotal;
        $export[]                = $finalRow;
     ...
     *
     * @return  ReportitemisedModel
     *
     * @since   6.0.0
     */
    private function createModel(): ReportitemisedModel
    {
        $model = new ReportitemisedModel(['ignore_request' => false]);

...
class Dispatcher extends AbstractModuleDispatcher implements HelperFactoryAwareInterface
{
    ...
    protected function getLayoutData(): array
    {
        ...
        // Load frontend language strings required by shared J2Commerce layouts
        ...
        // Build layout params: start from J2Commerce global config, overlay module settings
        $data['params']     = $this->buildLayoutParams($moduleParams);
        $data['itemId']     = (int) ($this->app->getMenu()->getActive()?->id ?? 0);
        $data['layoutType'] = $moduleParams->get('layout_type', 'grid');
        $data['moduleId']   = (int) $data['module']->id;

        // Load J2Commerce product list assets (core CSS/JS)
        try {
        ...
        // Load Swiper assets for slider layout
    ...
    {
        // Start from J2Commerce global component config so shared layouts get their expected params
        $baseParams = clone ComponentHelper::getParams('com_j2commerce');

        // Overlay module-specific settings using the param names expected by ProductLayoutService
        $baseParams->set('list_no_of_columns', (int) $moduleParams->get('list_no_of_columns', 3));
        ...
        // Carry through slider params for template use
